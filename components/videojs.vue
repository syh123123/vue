<template>
  <div class="videoBox" :id="'videoBox'+index" >
    <!-- <div :class="['top', hideControl ? 'hideTop' : '']">
      <div class="moreBox">
        ...
        <div class="topOpt" v-if="showTopOpt && qualityList.length > 0">
          <ul>
            <li  @click="showQuality = true;showTopOpt = false;">
...
export default {
  props: ["playInfo","index","playIndex",'data'],
  data() {
    return {
      redWidth: "0",
      player: "",
  ...
  methods: {
    showMenu(dom, data) {
      var _this = this;
      $(`#videoBox${this.index}`).on("touchstart", function (e) {
    ...
    readVideo(data) {
      ...
      if (data.videolow) {
        if (data.videolow) {
          this.qualityList.push({ src: data.videolow, name: "流畅" });
        }
        if (data.videomid) {
          this.qualityList.push({ src: data.videomid, name: "标清" });
        }
        if (data.videosrchd) {
          this.qualityList.push({ src: data.videosrchd, name: "高清" });
        }
        this.qualityIndex = [0];
        var src = this.qualityList[this.qualityIndex].src;
        var poster = this.qualityList[this.qualityIndex].poster||this.qualityList[this.qualityIndex].imgsrc;
        if (this.player) {
          this.player.src({ src: src });
        } else {
          this.showVideo({src:src,poster:poster?poster:''});
        }
      } else {
          this.showVideo({src:data.src,poster:data.poster?data.poster:''});
      }
    },
    showVideo(data) {
      var _this = this;
      this.player = videojs(_this.$refs['video'+_this.index], {
        loop: false,
        controls: true, //是否显示空间
        autoplay: false, //自动播放
        preload: "auto", //播放器加载完是否下载视频 （auto）浏览器支持即会下载  （metadata）仅加载视频元数据 （none）用户点击播放下载视频
        sources: [{ src: data.src }],
        poster:data.poster, //封面图
      });
      ...
      _this.player.on("pause", function () {
        _this.isPlay = false;
         _this.$emit("playVideoCallback", {type:'pause', playIndex: _this.index });
      });
      _this.player.on("play", function () {
        _this.isPlay = true;
         _this.$emit("playVideoCallback", {type:'play', playIndex: _this.index });
      });
    ...
    opation(type) {
      if (type == "prev") {} else {this.$emit("opation", {type: "next"});}
    },
    //倍数播放
